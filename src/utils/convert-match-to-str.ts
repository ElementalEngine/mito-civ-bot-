...
import { Civ6Player, Civ7Player } from "../types/reports.js";
...
export function convertMatchToStr(match: BaseReport): string {
  ...
  if (edition === "civ6") {
    ...
    body = playersSortedByPlacement
      .map(p => `<@${p.discord_id}>\t${p.user_name}\t${lookupCiv6Leader((p as Civ6Player).civ)}`)
      .join("\n");
  } else {
    const r = match as Civ7Report;
    meta = `Game: ${r.game} | Turn: ${r.turn} | Age: ${r.age} | Map: ${r.map_type} | Mode: ${r.game_mode}\n\n`;
    body = playersSortedByPlacement
      .map(p => `<@${p.discord_id}>\t${p.user_name}\t${lookupCiv7Civ((p as Civ7Player).civ)} ${lookupCiv7Leader((p as Civ7Player).leader)}`)
      .join("\n");
  }
  return meta + body;
}